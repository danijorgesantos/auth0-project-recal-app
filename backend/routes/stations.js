...
router.get('/',(req,res,next)=>{
  ...
  Station.find().select('coodinates returned collected').exec().then(docs=>{
      console.log(docs);
      const response={
        count:docs.length,
        users:docs.map(doc=>{
...
router.get('/:stationId',checkAuth,(req,res,next)=>{
  var url_parts = req.protocol + '://' + req.get('host') + req.originalUrl;
  const id=req.params.userId;
  Station.findById(id)
  .select('coodinates collected returned')
  .exec()
  .then(doc=>{
